...
const styles = StyleSheet.create({
  ...
  title: { fontSize: 18, fontWeight: 'bold', marginBottom: 5, textTransform: 'uppercase', color: '#047857' },
  ...
  totalValue: { width: 100, textAlign: 'right', fontWeight: 'bold', color: '#047857', fontSize: 14 },
  footer: { marginTop: 40, textAlign: 'center', fontSize: 9, color: '#9CA3AF' }
});

export function QuotePDFDocument({ quote }: { quote: any }) {
  const formatVND = (n: number) => new Intl.NumberFormat("vi-VN").format(Math.round(n)) + ' ' + (quote.currency || 'VND');
    ...
    <Document>
      <Page size="A4" style={styles.page}>
        <View style={styles.header}>
          ...
          <Text style={styles.subtitle}>Số / No: {quote.quoteCode || quote.id}</Text>
          <Text style={styles.subtitle}>Ngày / Date: {formatDate(quote.createdAt || new Date())}</Text>
        </View>
        ...
        <View style={styles.section}>
          ...
          <View style={styles.row}>
            ...
            <Text style={styles.colValue}>{quote.customer?.name || 'Khách lẻ'}</Text>
          </View>
          <View style={styles.row}>
            <Text style={styles.colLabel}>Email:</Text>
            <Text style={styles.colValue}>{quote.customer?.email || '—'}</Text>
          </View>
          <View style={styles.row}>
            <Text style={styles.colLabel}>Số điện thoại:</Text>
            <Text style={styles.colValue}>{quote.customer?.phone || '—'}</Text>
          </View>
        </View>
        ...
        <View style={styles.section}>
          ...
          <View style={styles.summaryRow}>
            ...
            <Text style={styles.summaryValue}>{formatVND(quote.subtotal)}</Text>
          </View>
          <View style={styles.summaryRow}>
            <Text style={styles.summaryLabel}>Chiết khấu (Discount):</Text>
            <Text style={styles.summaryValue}>-{formatVND(quote.discountAmount || 0)}</Text>
          </View>
          <View style={styles.summaryRow}>
            <Text style={styles.summaryLabel}>Phí vận chuyển (Shipping):</Text>
            <Text style={styles.summaryValue}>{formatVND(quote.shippingFee || 0)}</Text>
          </View>
          <View style={styles.summaryRow}>
            <Text style={styles.summaryLabel}>Thuế GTGT (VAT {quote.vatRate || 0}%):</Text>
            <Text style={styles.summaryValue}>{formatVND(quote.vatAmount || 0)}</Text>
          </View>
          <View style={[styles.summaryRow, { marginTop: 10 }]}>
            <Text style={styles.summaryLabel}>TỔNG CỘNG (TOTAL):</Text>
            <Text style={styles.totalValue}>{formatVND(quote.totalAmount)}</Text>
          </View>
        </View>
        ...
        <View style={styles.section}>
          ...
          {quote.paymentTerms && (
            <View style={styles.row}>
              ...
              <Text style={styles.colValue}>{quote.paymentTerms}</Text>
            </View>
          )}
          {quote.deliveryTerms && (
            <View style={styles.row}>
              <Text style={styles.colLabel}>Giao hàng:</Text>
              <Text style={styles.colValue}>{quote.deliveryTerms}</Text>
            </View>
          )}
          {quote.validUntil && (
            <View style={styles.row}>
              <Text style={styles.colLabel}>Hiệu lực báo giá:</Text>
              <Text style={styles.colValue}>{formatDate(quote.validUntil)}</Text>
            </View>
          )}
          {quote.commercialNotes && (
            <View style={[styles.row, { marginTop: 10 }]}>
              <Text style={styles.colValue}>{quote.commercialNotes}</Text>
            </View>
          )}
        </View>
